<footer class="footer">

   <section class="grid">

      <div class="box">
         <h3>quick links</h3>
         <a href="home.php"> <i class="fas fa-angle-right"></i> home</a>
         <a href="about.php"> <i class="fas fa-angle-right"></i> about</a>
         <a href="shop.php"> <i class="fas fa-angle-right"></i> shop</a>
         <a href="contact.php"> <i class="fas fa-angle-right"></i> contact</a>
      </div>

      <div class="box">
         <h3>extra links</h3>
         <a href="user_login.php"> <i class="fas fa-angle-right"></i> login</a>
         <a href="user_register.php"> <i class="fas fa-angle-right"></i> register</a>
         <a href="shop.php"> <i class="fas fa-angle-right"></i> services</a>
         <a href="orders.php"> <i class="fas fa-angle-right"></i> bookings</a>
      </div>

      <div class="box">
         <h3>contact us</h3>
         <a href="tel:1234567890"><i class="fas fa-mobile"></i> 09771925064</a>
         <a href="tel:11122233333"><i class="fas fa-phone"></i> 73730171</a>
         <a href="mailto:ezhomecareph@gmail.com"><i class="fas fa-envelope"></i> ezhomecareph@gmail.com</a>
      </div>

      <div class="box">
         <h3>follow us</h3>
         <a href="https://www.facebook.com/ezhomecareph" target="_blank"><i class="fab fa-facebook-f"></i>facebook</a>
      </div>

   </section>

   <div class="credit">&copy; copyright @ <?= date('Y'); ?> by <span>ByteMe Group</span> | all rights reserved!</div>

</footer>